...
class System(BusinessEntity):
    ...
    def __init__(self, entity_id : UUID, name : str, description : str) -> None:
        ...
        self._boundaries = [Boundary(uuid4(),'glabal boundary','')] #TODO this is not needed if its retrieved from database
        self._data_flows = []
    ...
    def _list_of_threats(self):
        return [threat for sub in (df.threats for df in list(set(self._data_flows).union(self.global_boundary.components))) for threat in sub]
